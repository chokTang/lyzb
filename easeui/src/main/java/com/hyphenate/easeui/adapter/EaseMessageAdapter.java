...
import com.hyphenate.easeui.widget.presenter.EaseChatGoodsPresenter;
...
public class EaseMessageAdapter extends BaseAdapter {
    ...
    private static final int MESSAGE_TYPE_GROUP_INFO = 14;//群组系统消息(XXX于XXXX创建群,XXXX被禁言)

    private static final int MESSAGE_TYPE_IMAGE = 14;
...
//            conversation.markAllMessagesAsRead();
    ...
    public int getItemViewType(int position) {
        ...
        if (message.getType() == EMMessage.Type.TXT) {
            ...
            } else if ("admin".equals(message.getStringAttribute(EaseConstant.EXTRA_FROM_ADMIN, ""))) {
                return MESSAGE_TYPE_GROUP_INFO;
            }

            if (message.getStringAttribute("msg_type", "").equals("msg_share")) {
    ...
    protected EaseChatRowPresenter createChatRowPresenter(EMMessage message, int position) {
        ...
        switch (message.getType()) {
            case TXT:
                if (message.getStringAttribute("msg_type", "").equals("msg_share")) {
                    presenter = new EaseChatGoodsPresenter();
                } else {
                    if (message.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_IS_BIG_EXPRESSION, false)) {
                        presenter = new EaseChatBigExpressionPresenter();
                    }else if("admin".equals(message.getStringAttribute(EaseConstant.EXTRA_FROM_ADMIN, ""))){
                        //EaseConstant.EXTRA_FROM_ADMIN 发送消息的文本  vaule="admin"为群主往自己的群发送消息
                        presenter = new EaseChatGroupPresenter();
                    }
                    else {
