    ...
    private void setUpBaseView() {
        ...
        if (userAvatarView != null) {
            ...
            if (message.direct() == Direct.SEND) {
                ...
                //自己发送的 头像
                String nickHeader = PreferenceUtil.getIntance().getString("user_hx_header", "");
                if (TextUtils.isEmpty(nickHeader)) {
                    nickHeader = message.getStringAttribute(EaseConstant.EXTRA_FROM_HEADER, message.getFrom());
                }
                LoadImageUtil.loadRoundImage(userAvatarView, nickHeader, 4, R.drawable.ease_default_avatar);
                if (usernickView != null) {
                    usernickView.setVisibility(View.GONE);
                }
            } else {
                //    EaseUserUtils.setUserAvatar(context, message.getFrom(), userAvatarView);
                String nickName = message.getStringAttribute(EaseConstant.EXTRA_FROM_NICKNAME, message.getFrom());
                String nickHeader = message.getStringAttribute(EaseConstant.EXTRA_FROM_HEADER, message.getFrom());
                if (TextUtils.isEmpty(nickHeader)) {
//                    Picasso.with(context).load(R.drawable.ease_default_avatar).into(userAvatarView);
                    LoadImageUtil.loadRoundImage(userAvatarView, R.drawable.ease_default_avatar, 4, R.drawable.ease_default_avatar);
                } else {
//                    Picasso.with(context).load(nickHeader).placeholder(R.drawable.ease_default_avatar).into(userAvatarView);
                    LoadImageUtil.loadRoundImage(userAvatarView, nickHeader, 4, R.drawable.ease_default_avatar);
                }
                //设置在线客户的头像
                if (nickName.equals("在线客户")) {
                    LoadImageUtil.loadRoundImage(userAvatarView, R.mipmap.ic_launcher, 4, R.drawable.ease_default_avatar);
                }
