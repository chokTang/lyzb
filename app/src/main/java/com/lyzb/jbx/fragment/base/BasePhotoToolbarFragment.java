...
public abstract class BasePhotoToolbarFragment<P extends BasePresenter> extends BaseToolbarFragment<P>
    ...
    /**
     * 从相册选择一张图片并裁剪成圆形/方形 指定尺寸
     */
    protected void onChooseFromPhotoRound(CropOptions options, ISelectPictureListener listener) {
        type_select = SELECT_PHOTO_ROUND;
        mListener = listener;
        Uri uri = AppUtil.getUriForFile(getContext(), FileUtil.createFile(getContext().getPackageName(), System.currentTimeMillis() + ".jpg"));
     ...
     * @param listener
     */
    protected void onChooseFromCameraRound(CropOptions options, ISelectPictureListener listener) {
        type_select = SELECT_CAMERA_ROUND;
        mListener = listener;
        Uri uri = AppUtil.getUriForFile(getContext(), FileUtil.createFile(getContext().getPackageName(), System.currentTimeMillis() + ".jpg"));
