 ...
 * 动态列表页面
 */
public class MyCardDynamicListAdapter extends BaseRecyleAdapter<DynamicModel> {
    ...
    private boolean mIsFollow = true;//默认是显示关注或者未关注状态

    public void setFollow(boolean follow) {
        mIsFollow = follow;
    }

    public MyCardDynamicListAdapter(Context context, List<DynamicModel> list) {
    ...
    @Override
    protected void convert(final BaseViewHolder holder, DynamicModel item) {
        switch (holder.getItemViewType()) {
            ...
            case TYPE_PICTORE:
                ...
                if (item.getFileList().size() == 0) {
                    ...
                    pictureHodler.setTag(R.id.layout_first, item.getFileList().get(0).getFile());
                } else {
                    pictureHodler.setVisible(R.id.layout_first, false);
            ...
            case TYPE_VIDEO:
                ...
                AutoLinkTextView tv_content_value1 = videoHolder.cdFindViewById(R.id.tv_content_value);
                TextView tv_max1 = videoHolder.cdFindViewById(R.id.tv_max);
        ...
        public PictureHodler(final Context context, View itemView) {
            ...
            layout_first.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    List<String> urllist = new ArrayList<>();
                    urllist.add(v.getTag().toString());
                    ImageUtil.Companion.statPhotoViewActivity(context, 0, urllist);
                }
            });
        }
    }
    ...
    private void headerInit(BaseViewHolder holder, DynamicModel model) {
        ...
        if (!mIsFollow || model.getCreateMan().equals(App.getInstance().userId)) {
