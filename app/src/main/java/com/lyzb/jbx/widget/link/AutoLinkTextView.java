...
public class AutoLinkTextView extends android.support.v7.widget.AppCompatTextView {
     ...
     * @return
     */
    private List<AutoLinkItem> detailFirst(List<AutoLinkItem> linkItems) {
        List<AutoLinkItem> list = new LinkedList<>();
        boolean isLjFirst = true;
        for (AutoLinkItem item : linkItems) {
            if (item.getAutoLinkMode() == AutoLinkMode.MODE_CUSTOM) {
                if (isLjFirst) {
                    list.add(item);
                }
                isLjFirst = false;
    ...
    private List<AutoLinkItem> matchedRanges(CharSequence text) {
        ...
        return detailFirst(autoLinkItems);
    }
