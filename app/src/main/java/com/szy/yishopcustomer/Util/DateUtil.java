...
public class DateUtil {
    ...
    public static String format(long timeStamp) {
        ...
        Date mCureentStartDate = StringToDate(mCurrentStartDateStr, DateStyle.YYYY_MM_DD);
        if (mCureentStartDate.getTime() <= timeStamp) {
            return DateToString(new Date(timeStamp), DateStyle.HH_MM);
        } else if (timeStamp >= addDay(mCureentStartDate, -1).getTime()) {
            return "昨天 " + DateToString(new Date(timeStamp), DateStyle.HH_MM);
        } else {
            return DateToString(new Date(timeStamp), DateStyle.YYYY_MM_DD_HH_MM);
        }
    }
}
