...
import static com.szy.yishopcustomer.Fragment.InvoiceInfoFragment.EMAIL_INVOICE_POSITION;
import static com.szy.yishopcustomer.Fragment.InvoiceInfoFragment.NORMAL_INVOICE_POSITION;
import static com.szy.yishopcustomer.Fragment.InvoiceInfoFragment.VAT_INVOICE_POSITION;
 ...
 * 普通商品结算页
 */
public class CheckoutFragment extends CommonPayFragment implements TextView
        ...
        switch (RequestCode.valueOf(requestCode)) {
            ...
            case REQUEST_PICK:
                if (data != null && resultCode == Activity.RESULT_OK) {
                    ...
                    if (position >= 0) {
                        List<ShipItemModel> tempList = (List<ShipItemModel>) mCheckoutAdapter
                                .data.get(position);

                        PickUpModel tPickUpModel = mPickUpList.get(tempList.get(listPostion)
                                .shop_id).get(itemPosition);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mCheckoutAdapter.onCheckedChangeListener = new RadioGroup.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(RadioGroup radioGroup, @IdRes int i) {
                ...
                List<ShipItemModel> tempList = (List<ShipItemModel>) mCheckoutAdapter.data.get
                        (position);
                if ("1".equals(tempList.get(i).id)) {
                    ...
                    if (TextUtils.isEmpty(tempList.get(i).pickup_name)) {
                        openPickUpActivity(position, i, "", tempList.get(i).shop_id);
                    } else {
                        changeDeliveryMode(tempList.get(i));
                    }
                } else {
...
//                mCheckoutAdapter.notifyDataSetChanged();
    ...
    public void openPickUpActivity(int postion, int listPostion, String pickid, String shopid) {
        ...
        intent.putParcelableArrayListExtra(Key.KEY_PICKUP_LIST.getValue(), mPickUpList.get(shopid));
...
//            intent.putExtra(Key.KEY_ORDER_PAY_SUCCESS.getValue(), success);
    ...
    private void changeInvoice(InvoiceResultModel result) {
        ...
        if (result.inv_type == 0) {
        ...
        } else if (result.inv_type == NORMAL_INVOICE_POSITION) {

            Map<String, String> temp = JSON.parseObject(mModel.data.invoice_info.get(NORMAL_INVOICE_POSITION).contents,
                    Map.class);
            temp.put("inv_title", result.inv_title);
            temp.put("inv_company", result.inv_company);
            temp.put("inv_content", result.inv_content + "");
            temp.put("inv_taxpayers", result.inv_taxpayers + "");
            mModel.data.invoice_info.get(NORMAL_INVOICE_POSITION).contents = JSON.toJSONString(temp);

            for (int i = 0; i < mModel.data.invoice_info.size(); i++) {
                InvoiceItemModel invoiceItemModel = mModel.data.invoice_info.get(i);
                if (NORMAL_INVOICE_POSITION == i) {
            ...
            for (int i = 0; i < mModel.data.invoice_info.get(NORMAL_INVOICE_POSITION).content_list.size(); i++) {
                ContentItemModel contentItemModel = mModel.data.invoice_info.get(NORMAL_INVOICE_POSITION).content_list.get(i);
                if (i == result.inv_content) {
        ...
        } else if (result.inv_type == VAT_INVOICE_POSITION) {
            List<Map<String, String>> list = JSON.parseObject(mModel.data.invoice_info.get(VAT_INVOICE_POSITION)
                    .contents, List.class);
            ...
            mModel.data.invoice_info.get(VAT_INVOICE_POSITION).contents = JSON.toJSONString(list);

            for (int i = 0; i < mModel.data.invoice_info.size(); i++) {
                InvoiceItemModel invoiceItemModel = mModel.data.invoice_info.get(i);
                if (VAT_INVOICE_POSITION == i) {
                    invoiceItemModel.selected = "selected";
                } else {
                    invoiceItemModel.selected = "";
                }
        ...
        }else if (result.inv_type == EMAIL_INVOICE_POSITION){
            Map<String, String> temp = JSON.parseObject(mModel.data.invoice_info.get(EMAIL_INVOICE_POSITION).contents,
                    Map.class);
            temp.put("inv_title", result.inv_title);
            temp.put("inv_email", result.inv_email);
            temp.put("inv_company", result.inv_company);
            temp.put("inv_content", result.inv_content + "");
            temp.put("inv_taxpayers", result.inv_taxpayers + "");
            mModel.data.invoice_info.get(EMAIL_INVOICE_POSITION).contents = JSON.toJSONString(temp);

            for (int i = 0; i < mModel.data.invoice_info.size(); i++) {
                InvoiceItemModel invoiceItemModel = mModel.data.invoice_info.get(i);
                if (EMAIL_INVOICE_POSITION == i) {
                    invoiceItemModel.selected = "selected";
                } else {
                    invoiceItemModel.selected = "";
                }
            ...
            for (int i = 0; i < mModel.data.invoice_info.get(EMAIL_INVOICE_POSITION).content_list.size(); i++) {
                ContentItemModel contentItemModel = mModel.data.invoice_info.get(EMAIL_INVOICE_POSITION).content_list.get(i);
                if (i == result.inv_content) {
                    contentItemModel.checked = "checked";
                } else {
                    contentItemModel.checked = "";
            ...
            request.add("inv_name", result.inv_name);
            request.add("inv_title", result.inv_title);
            request.add("inv_email", result.inv_email);
            request.add("inv_company", result.inv_company);
            request.add("inv_taxpayers", result.inv_taxpayers);
            request.add("inv_content", result.inv_content);
        }
    ...
    private void changeDeliveryMode(ShipItemModel result) {
        ...
        request.add("balance", getBalance());
        request.add("balance_enable", isBalanceEnabled());
        request.add("shipping_list[0][shop_id]", result.shop_id);
        request.add("shipping_list[0][shipping_id]", result.id);
        request.add("shipping_list[0][pickup_id]", result.pickup_id);
        request.setAjax(true);
        addRequest(request);
    }
    ...
    private void changePaymentCallback(String response) {
        HttpResultManager.resolve(response, ChangePaymentModel.class,
                new HttpResultManager.HttpResultCallBack<ChangePaymentModel>() {
                    @Override
...
//                                    )) {
//                                        mModel.data.cart_info.shop_list.get(entry.getKey()).order.order_amount_format = changePaymentModel.shop_orders.get(iEntry.getKey()).order_amount_format;
//                                    }
//                                }
//                            }
//                        }
                        ...
                        if (orderInfoModel == null) {
                            ...
                            for (int i = 0; i < mCheckoutAdapter.data.size(); i++) {
                                if (mCheckoutAdapter.data.get(i) instanceof BalanceInputModel) {
                                    mCheckoutAdapter.data.add(i - 1, orderInfoModel);
                                    mCheckoutAdapter.data.add(i - 1, new CheckoutDividerModel());
                                    mCheckoutAdapter.notifyItemRangeChanged(i - 1, mCheckoutAdapter.data
                                            .size() - 1);
//                            mCheckoutAdapter.notifyDataSetChanged();
                                    return;
                                }
                            ...
                            for (int i = 0; i < mCheckoutAdapter.data.size(); i++) {
                                if (mCheckoutAdapter.data.get(i) instanceof OrderInfoModel) {
                                    OrderInfoModel orderInfoModel = (OrderInfoModel) mCheckoutAdapter
                                            .data.get(i);
                                    orderInfoModel.shipping_fee_format = changePaymentModel.order
                                            .shipping_fee_format;
                                    ...
                                    orderInfoModel.shop_store_card_amount_format = changePaymentModel
                                            .order.shop_store_card_amount_format;
                                    orderInfoModel.shop_store_card_amount = changePaymentModel
                                            .order.shop_store_card_amount;
                                    orderInfoModel.full_cut_amount_format = changePaymentModel
                                            .order.full_cut_amount_format;
                                    orderInfoModel.full_cut_amount = changePaymentModel
                                            .order.full_cut_amount;
                                }
                            }
                        }
    ...
    private void refreshCallback(String response) {
        LogUtils.Companion.e("发票数据"+response);
    ...
    private void setUpAdapterData() {
        //设置总金额
        mTotalPriceTextView.setText(mModel.data.cart_info.order.order_amount_format);
        ...
        //如果没有选中的地址，需要默认选中第一个，所以加这个变量
        AddressItemModel firstAddressModel = null;
        String floatAddress = "";
        if (mModel.data.address_list != null && mModel.data.address_list.size() > 0) {
            int i = 0;
            for (AddressItemModel addressItemModel : mModel.data.address_list) {
                if (i == 0) {
                    firstAddressModel = addressItemModel;
                }
                //如果有选中的地址
                if (addressItemModel.selected == 1) {
                    mCheckoutAdapter.data.add(addressItemModel);
                    addressSelected = true;
                    floatAddress = addressItemModel.region_name + addressItemModel.address_detail;
            ...
            //如果没有选中的地址，则指定第一个为默认选中
            if (!addressSelected) {
                mCheckoutAdapter.data.add(firstAddressModel);
                addressSelected = true;
                floatAddress = firstAddressModel.region_name + firstAddressModel.address_detail;
                if (Utils.isNull(firstAddressModel.address_lat)) {
                    showConfirmDialog(R.string.addressDialogLabel, ViewType
                            .VIEW_TYPE_ADDRESS_CHOICE.ordinal(), 0, Integer.valueOf
                            (firstAddressModel.address_id));
        ...
        if (mPickUpList == null) {
            mPickUpList = new HashMap<String, ArrayList<PickUpModel>>();
        } else {
            mPickUpList.clear();
        }
        ...
        for (Map.Entry<String, ShopItemModel> entry : mModel.data.cart_info.shop_list.entrySet()) {
            ...
            ArrayList<PickUpModel> temp = new ArrayList<>();
            if (shopItemModel.shop_info.pickup_list != null) {
                temp.addAll(shopItemModel.shop_info.pickup_list);
                mPickUpList.put(entry.getKey(), temp);
            }
...
//                    shopItemModel.addGoodListForActList();
                    for (GoodsItemModel goodsItemModel : shopItemModel.goods_list) {

                        //如果没有选择地址，不显示“该商品不支持当前配送地址”
                        if (!addressSelected) {
                            goodsItemModel.limit = false;
                        } else {
                            if (shopItemModel.limit_goods_ids != null) {
                                goodsItemModel.limit = shopItemModel.limit_goods_ids.contains
                                        (goodsItemModel.goods_id);
                            } else {
                                goodsItemModel.limit = false;
                            }
                        }
                ...
                if (shopItemModel.shipping_list == null || shopItemModel.shipping_list.size() <=
                        0) {
                    ...
                    for (ShipItemModel shipItemModel : shopItemModel.shipping_list) {
                        ...
                        shipItemModel.price = shopItemModel.order.shipping_fee_format;
                    }
                ...
                if (!Utils.isNull(shopItemModel.bonus_list)) {
                    ...
                    for (BonusItemModel bonusItemModel : shopItemModel.bonus_list) {
                        ...
                        if (!Utils.isNull(bonusItemModel.selected)) {
                            if (bonusItemModel.selected.equals("selected")) {
                                groupModel.selectedItem = bonusItemModel.bonus_name;
                            }
                        }
                    }
                    groupModel.isShopBouns = true;
                    mCheckoutAdapter.data.add(groupModel);
                }
                ...
                shopItemModel.order.shop_integral_num = shopItemModel.shop_integral_num;
        ...
        if (mModel.data.send_time_list.size() != 0) {
        ...
        if (mModel.data.cart_info.invoice_type != 0) {
            //取出发票信息
            groupModel = new GroupModel(getString(R.string.invoiceInfo), getString(R.string
            ...
            groupModel.selectedItem = mModel.data.invoice_desc.get(0).toString();
            mInvoiceGroupModel = groupModel;
            mCheckoutAdapter.data.add(groupModel);

            //取出不能开发票的商品
            if (mModel.data.invoice_info.size() > 0 && !Utils.isNull(mModel.data.invoice_info.get
                    (0).goods_list)) {
        ...
        //订单信息
        if (orderInfoModel == null) {
            orderInfoModel = mModel.data.cart_info.order;
            orderInfoModel.order_integram_num = mModel.data.cart_info.order_integram_num;

            mCheckoutAdapter.setOrderInfo(orderInfoModel);
            mCheckoutAdapter.data.add(orderInfoModel);
            mCheckoutAdapter.data.add(new CheckoutDividerModel());
        }
        ...
        if (Double.parseDouble(userInfoModel.balance) > 0) {
...
//        if (!Utils.isNull(payItemModel) && payCode.equals("app_weixin") && !Utils.isWeixinAvilible(getActivity())) {
        ...
        HttpResultManager.resolve(response, SubmitModel.class, new HttpResultManager
            ...
            public void onSuccess(SubmitModel submitModel) {
                EventBus.getDefault().post(new CommonEvent(EventWhat.EVENT_CHECKOUT_SUCCEED
                        .getValue()));
                //在线支付支付完成后产生group_sn,此处group_sn=0
