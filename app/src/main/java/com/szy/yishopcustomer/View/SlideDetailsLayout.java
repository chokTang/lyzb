...
public class SlideDetailsLayout extends ViewGroup {
    ...
    private static final float DEFAULT_PERCENT = 0.05f;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        final int childHeightMeasureSpec = MeasureSpec.makeMeasureSpec(pHeight, MeasureSpec
                .EXACTLY);
        ...
        switch (aciton) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (canChildScrollVertically((int) yDiff)) {
                    ...
                    if (yDiffabs > mTouchSlop && yDiffabs >= xDiffabs && !(mStatus == Status
                            .CLOSE && yDiff > 0 || mStatus == Status.OPEN && yDiff < 0)) {
    ...
    private void animatorSwitch(final float start, final float end, final boolean changed, final
    long duration) {
    ...
    protected boolean canListViewSroll(AbsListView absListView) {
        if (mStatus == Status.OPEN) {
            ...
            return count > 0 && (absListView.getLastVisiblePosition() < count - 1 || absListView
                    .getChildAt(count - 1).getBottom() > absListView.getMeasuredHeight());
        }
    }
